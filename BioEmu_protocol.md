...
## BioEmu no Colab Notebook - Gerando Conformações a partir de FASTA

### 1. Número de Conformações e Grupo Amostral, e Estratégia (Colab Free)
...
### 2. Etapas e Arquivos Gerados pelo BioEmu

* **`Write samples and run foldseek`:** gera conformações e clusterização de estruturas

    * O BioEmu gera n conformações (que você define) onde ele prediz estruturas da proteína emulando seu 'ensembl' termodinâmico (eles usam a distribuição Boltzmann, o suplementar do paper deles explica bastante coisa). Por default **estruturas fisico-quimicamente impossíveis de existir** são deletadas, então é bem comum o número de estruturas final não ser exatamente o número inicialmente proposto
    (dá pra mudar isso, mas não era do interesse nesse caso). Essa etapa gera o primeiro output do BioEmu, que são: 
        * **Arquivos `.pdb` individuais para cada uma das (n) conformações que foram preditas como parte do ensembl.** 
        * **Clusterização de estruturas com o foldseek, representadas em arquivo .json.**
            ...
            * Um exemplo de arquivo .json usando less em terminal bash: 
...
* **Organização de Diretórios de Output:** O resultado gerado pelo BioEmu é extenso, mas com uma organização bastante padronizada, com folders de nomenclatura padronizada comportando os arquivos de saída do processo da ferramenta. 
    ...
    * **Folder `pdb_samples`**:
        * O diretório pdb_samples contém todas as estruturas individuais em formato `.pdb`. Pelo colab, ela é nomeada de forma padrão. O diretório sempre tem como nome `pdb_samples`, e os arquivos com as estruturas no interior são nomeadas `sample_X.pdb`.

        * Exemplo de organização de `pdb_samples`: 
        ```bash
    ...
    * **Folder `foldseek` e `cg_coefficients`:** diretórios de chamada de ferramentas para processamento da análise feita pelo BioEmu, e não possuem nenhum arquivo significativo. Já os diretórios `9J7V_cc11a` e `pdb_samples` vão conter os dados de trajetória, topologia e cada um dos arquivos .pdb gerados. 
---

### 3. Trajetória e Topologia - Arquivos XTC e .PDB no BioEmu
...
* **Folder de Análise e arquivos de interesse:** 
    * Conforme explicado no `README.md`, no tópico `#usage` do benchmark, o BioEmu gera arquivos em formato `.xtc` para serem interpretados como `mdtraj.Trajectory` (trajetória). Além disso, todo `.xtc` obrigatoriamente é acompanhado por um arquivo de *topologia*. No caso do BioEmu, A topologia é definida por um arquivo em formato `.pdb` onde o nome do arquivo **contém obrigatoriamente topology** escrito, e **encontra-se no mesmo diretório**. 
...
### 4. Upload de Outputs do BioEmu (Notebook Colab) pelo Google Drive

```python

#Dentro do colab que gerou as amostras, adicione duas abas de código: 

## ABA 1 - montagem de drive
from google.colab import drive
drive.mount('/content/drive')

##ABA 2 - Upload das pastas de output do BioEmu pra uma pasta no drive conectado 
!cp -r /content/[sua_amostra]_cc11a /content/drive/MyDrive/
!cp -r /content//pdb_samples /content/drive/MyDrive/

```

* **NOTA1:** Indico criar e direcionar esses arquivos pra uma pasta no drive, pra não se perderem lá dentro. Você vai precisar baixar alguns dos arquivos pro seu computador ou diretamento pro servidor posteriormente, então é interessante ter eles de fácil acesso.
...
### 5. Cálculo RMSD/RMSF dos outputs do BioEmu - GROMACS

* **NOTA DE OTIMIZAÇÃO:** Por muita tentativa e erro, cheguei em 2 processos gerais executáveis que podem ser usados de forma simples pra trabalhar com esses arquivos do BioEmu (e futuramente, com arquivos de dinãmica num geral): 
    * [`bash executável 1`]() #Adicionar exemplos de uso em oto canto (acho, talvez fique poluído aqui)
    * [`bash executável 2`]()

* **Sobre Instalação do GROMACS e Interface de Processamento:**
    ...
    * Ativação de ambiente conda com GROMACS (exemplo):
    ```bash
...
* **Cálculo RMSD e RMSF com GROMACS - para os output BioEmu**:

    * Referências de Escolhas de Opções de `Least Square Fit` e `Group For Output` tiradas das recomendações do próprio [Tutorial Gromacs](http://www.mdtutorials.com/gmx/lysozyme/09_analysis.html) 
        * **`LEAST SQUARE FIT`**: Seleção de estrutura estável para representar o fold de referência para o cálculo do RMSD. Por isso a escolha do Backbone.
...
```bash
    # COMANDO 1: Variabilidade do ensemble completo EM RELAÇÃO Á ESTRUTURA INICIAL QUERY - RMSD

    gmx rms -s topology.pdb -f samples.xtc -o ensembl-rmsd
    ...
    ### Escolha a opção ***4*** para ambas, de `BACKBONE`

    # COMANDO 2: RMSF do ensemble completo EM RELAÇÃO Á ESTRUTURA INICIAL QUERY

    gmx rmsf -s topology.pdb -f samples.xtc -o example-rmsf -res
    # não esqueçam o parâmetro -res no final, ele é essencial para medir a flutuaçãoa nível de resíduo.
    ## PRO RMSF, SÓ SE PEDE O LEAST SQUARE FIT. É 4:('BACKBONE').
```

--- 

### 6. `.XTC` para `.PDB` e Transformação de trajetórias para grafo utilizando RING 4.0

* **Conversão `.XTC` para .`.PDB` com GROMACS - `gmx trjconv`**:
    ...
    * Script bash de Conversão com `trjconv:`
    ```bash
        # Conversão de .xtc para .pdb do dado de trajetória - BioEmu
        gmx trjconv -s topology.pdb -f samples.xtc -o traj.pdb

        ## SELEÇÃO DE OPÇÕES:  A SELEÇÃO A SER FEITA É DE 0:['System']. Diferente dos comandos anteriores, aqui o interesse é de referência a estrutura completa para computar a trajetória.
    ```
* **Comando RING em terminal Bash:**
    * Para rodar a ferramenta do RING em terminal para *todas as conformações da trajetória*, eu utilizei o seguinte comando:
    ...
    ```bash
        # RING4.0 para TODAS as conformações de .pdb
        ring -i i arquivo-de-trajetória.pdb --all_edges --all_models --out_dir diretório-de-escolha
